...
默认六方向计划从每日核心源加三日滚动源的候选池中，为每个方向选择适配的中英文来源；以下不是逐日全部执行的承诺：

- 核心源：Reddit、X、YouTube、小红书、知乎。
- 滚动组一：TikTok、Instagram、抖音。
- 滚动组二：LinkedIn、Threads、B站。
- 滚动组三：TikTok、Instagram、微信搜一搜。
- 定向地区查询使用地区的本地语言表达，不只追加国家名；默认六方向分别保留中文和英文任务。

这里导出的 `tikhub-search-plan.json` 不代表已批准付费。用户授权一次性跨方向发现预算后，可通过 `resume --discover --max-cost-usd` 实时估价并执行受控批次；没有授权时只用于规划。初始化发现无需先有候选 ID，仍须保留方向、查询及发现目标。见 [六方向发现](cross-industry-discovery.md)。
...
发现与补证的预算分配以用户本次授权为准，不强制旧的 20% 比例；累计费用不得超过同轮明确上限。每批结束后，Agent 必须判断是否新增相关用户行为、探索线索、BENCH、合格结论或关键门槛证据；无产出则停止原查询并诊断原因。执行器只检查请求数、预算和响应状态，不能自动判断商业价值。“每来源 3 请求”的硬限制针对定向搜索补证；评论计划仍采用前述 1–5 帖、每帖 2 请求的独立限制。
