...
| Hacker News、GitHub | 本 Skill 内置 `community_query.py` 公开只读适配器 | 固定官方公开端点；GitHub Token 可选；结果直接规范化，不保存完整响应 |
...
商业信号是建立 `BENCH` 的主入口。社交热度只能说明关注度；没有产品、付款者与付款证据时不得建立正式付费对标。
...
地区轮换用于生成 R/SIG 迁移池，不要求每条都做深度分析。只有出现目标地区直接付款证据时，才允许进入 A/B 级 OPP。
