...
def main(argv=None) -> int:
    ...
    try:
        if args.types:
            ...
            result = write_site_bundle([json.loads(p.read_text()) for p in args.site_report], args.output, site_id=args.site_id)
        elif args.report and args.output:
            result = write_public_bundle(json.loads(args.report.read_text()), args.output)
        else:
