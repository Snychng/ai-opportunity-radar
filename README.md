...
v4.5 增加[多模型协作搜索](references/multi-model-search.md)：Grok 可选负责 X，宿主 AI 或已有其他联网模型搜索网页与社区。使用 Grok 前需要先通过 Hermes 完成订阅 OAuth 登录并配置认证文件路径；没有 Grok 的用户正常使用有预算授权的 TikHub 或宿主 X 搜索。检索候选与已核验原文分开保存，多模型发现同帖会合并；各路独立执行，由主协调者统一提交研究。
